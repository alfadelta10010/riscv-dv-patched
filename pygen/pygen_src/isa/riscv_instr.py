    ...
    def create_instr_list(cls, cfg):
        ...
        for instr_name, instr_group in cls.instr_registry.items():
            ...
            if ((riscv_reg_t.SP in cfg.reserved_regs) and
                ...
                continue
            # src/isa/riscv_instr.sv:110-111 reads
            #
            #   if (!cfg.enable_sfence && instr_name == SFENCE_VMA) continue;
            #   if (cfg.no_fence && (instr_name inside {FENCE, FENCE_I, SFENCE_VMA})) continue;
            #
            # The port dropped the negation from the first and the guard from
            # the second. The second one is the damaging half: with no
            # condition on it, FENCE and FENCE_I were struck from the
            # instruction set unconditionally and could never be generated by
            # any test, whatever no_fence said. FyraCore implements Zifencei
            # and asserts fencei_flush on FENCE.I, and none of it was reachable
            # from this generator.
            #
            # The two have to be corrected together: restoring the guard on the
            # second line without the negation on the first would start
            # emitting SFENCE_VMA, which this core does not implement.
            if not cfg.enable_sfence and instr_name == riscv_instr_name_t.SFENCE_VMA:
                continue
